"use client";

import { useRef, useState, lazy, Suspense, type CSSProperties } from "react";
import { motion } from "framer-motion";
import { Check, Download, LoaderCircle } from "lucide-react";
import type { InvitationData } from "./InvitationDataLoader";
import { formatPublicEventDate } from "../lib/eventDate";
import {
  buildPublicRsvpUrl,
  publicBaseUrlFromLocation,
  publicRsvpIdentifierFromLocation,
} from "../lib/rsvpUrl";

// Dynamic imports — only loaded after RSVP confirm (code splitting)
const QRCodeSVG = lazy(() =>
  import("qrcode.react").then((m) => ({ default: m.QRCodeSVG })),
);

/* ── Catalyst logomark (icon only) ──────────────────────────────────── */
function EventiAppMark({
  className,
  style,
}: {
  className?: string;
  style?: CSSProperties;
}) {
  return (
    <svg
      viewBox="0 0 26 22"
      fill="currentColor"
      className={className}
      style={style}
      aria-hidden="true"
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M6.999.5L6.57.743.57 10.743v.514l6 10 .429.243H19l.353-.854L16.853 18.146 16.499 18H9.274L4.841 11l4.433-7H16.499l.354-.146 2.5-2.5L19 .5H6.999Z"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M20.793 4.219l-2.427 2.427-.069.621 2.364 3.732-2.364 3.733.069.621 2.427 2.427.783-.096 3.856-6.427v-.514l-3.856-6.427-.783-.097Z"
      />
    </svg>
  );
}

interface Props {
  invData: InvitationData;
  token: string;
}

const cardStyle: CSSProperties = {
  background: "var(--eventi-color-surface, #ffffff)",
  boxShadow: "0 4px 24px rgba(0,0,0,0.08)",
};

const accentBarStyle: CSSProperties = {
  background:
    "linear-gradient(90deg, var(--eventi-color-accent, #C7A44C), var(--eventi-color-border, #E8D5A0), var(--eventi-color-accent, #C7A44C))",
};

const brandStyle: CSSProperties = {
  color: "var(--eventi-color-accent, #C7A44C)",
};

const headingStyle: CSSProperties = {
  color: "var(--eventi-color-heading, #07293A)",
  fontFamily:
    "var(--eventi-font-heading-effective, 'Astralaga SemiBold', serif)",
};

const mutedStyle: CSSProperties = {
  color: "var(--eventi-color-muted, #8B5D3D)",
  fontFamily: "var(--eventi-font-body-effective, Aloevera Display, sans-serif)",
};

const dividerLineStyle: CSSProperties = {
  backgroundColor: "var(--eventi-color-border, #E8D5A0)",
};

const dividerDiamondStyle: CSSProperties = {
  backgroundColor: "var(--eventi-color-accent, #C7A44C)",
};

const qrFrameStyle: CSSProperties = {
  background: "var(--eventi-color-surface, #FAFAF8)",
  border: "1.5px solid var(--eventi-color-border, #E8D5A0)",
};

const hintStyle: CSSProperties = {
  color: "var(--eventi-color-muted, #9ca3af)",
};

const saveButtonStyle: CSSProperties = {
  background:
    "linear-gradient(135deg, var(--eventi-color-heading, #07293A), var(--eventi-color-body, #0D3D56))",
  color: "var(--eventi-color-border, #E8D5A0)",
};
export default function RSVPConfirmationCard({ invData, token }: Props) {
  const cardRef = useRef<HTMLDivElement>(null);
  const [saveState, setSaveState] = useState<
    "idle" | "saving" | "saved" | "error"
  >("idle");
  // Build the invitation URL — QR points back to the event page with token
  const browserLocation =
    typeof window !== "undefined" ? window.location : undefined;
  const publicBaseUrl = publicBaseUrlFromLocation(browserLocation);
  const currentIdentifier =
    publicRsvpIdentifierFromLocation(browserLocation) ||
    invData.eventIdentifier;
  const eventUrl = buildPublicRsvpUrl(publicBaseUrl, token, currentIdentifier);
  const formattedDate = formatPublicEventDate(
    invData.eventDate,
    invData.eventTimezone,
  );

  const handleSaveImage = async () => {
    if (!cardRef.current || saveState === "saving") return;
    setSaveState("saving");
    try {
      const { toPng } = await import("html-to-image");
      const dataUrl = await toPng(cardRef.current, {
        quality: 0.95,
        pixelRatio: 3,
      });
      const link = document.createElement("a");
      const safeGuestName =
        invData.guestName
          .normalize("NFD")
          .replace(/[\u0300-\u036f]/g, "")
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/^-+|-+$/g, "") || "invitado";
      link.download = `confirmacion-${safeGuestName}.png`;
      link.href = dataUrl;
      link.click();
      setSaveState("saved");
    } catch (e) {
      console.error("[RSVPCard] save image error", e);
      setSaveState("error");
    }
  };
  const eventName = invData.eventName;

  const whatsappText = encodeURIComponent(
    `¡Confirmé mi asistencia!\n${eventName ? `${eventName}\n` : ""}${formattedDate}\n\nMi invitación: ${eventUrl}`,
  );

  return (
    <motion.div
      initial={{ opacity: 0, y: 24 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, ease: "easeOut" }}
      className="flex flex-col items-center gap-5 mt-8"
    >
      {/* Card — captured by html-to-image */}
      <div
        ref={cardRef}
        className="w-full max-w-80 rounded-3xl overflow-hidden flex flex-col items-center text-center"
        style={cardStyle}
      >
        {/* Gold accent bar */}
        <div className="w-full h-1.5" style={accentBarStyle} />

        <div className="px-8 py-7 flex flex-col items-center gap-4 w-full">
          {/* Brand mark */}
          <div className="flex items-center gap-1.5">
            <EventiAppMark className="h-3.5 w-auto" style={brandStyle} />
            <span
              className="text-[10px] uppercase tracking-[0.2em] font-aloevera font-medium"
              style={brandStyle}
            >
              eventiapp
            </span>
          </div>

          {/* Header */}
          <div>
            <p
              className="text-xs uppercase tracking-[0.15em] font-aloevera"
              style={mutedStyle}
            >
              Confirmacion de asistencia
            </p>
            {eventName && (
              <h3
                className="text-xl font-astralaga mt-1.5"
                style={headingStyle}
              >
                {eventName}
              </h3>
            )}
          </div>

          {/* Divider */}
          <div className="flex items-center gap-3 w-full px-4">
            <div className="flex-1 h-px" style={dividerLineStyle} />
            <div
              className="w-1.5 h-1.5 rotate-45"
              style={dividerDiamondStyle}
            />
            <div className="flex-1 h-px" style={dividerLineStyle} />
          </div>

          {/* Guest name */}
          <p
            className="text-2xl font-astralaga leading-tight"
            style={headingStyle}
          >
            {invData.guestName}
          </p>

          {/* Date */}
          {formattedDate && (
            <p className="text-sm font-aloevera capitalize" style={mutedStyle}>
              {formattedDate}
            </p>
          )}

          {/* QR Code with branded frame */}
          <div className="p-4 rounded-2xl mt-1" style={qrFrameStyle}>
            <Suspense
              fallback={
                <div className="w-28 h-28 bg-gray-100 animate-pulse rounded-lg" />
              }
            >
              <QRCodeSVG
                value={eventUrl}
                size={112}
                fgColor="#07293A"
                bgColor="#FAFAF8"
                level="M"
                imageSettings={{
                  src:
                    "data:image/svg+xml," +
                    encodeURIComponent(
                      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 26 22" fill="%23C7A44C"><path fill-rule="evenodd" clip-rule="evenodd" d="M6.999.5L6.57.743.57 10.743v.514l6 10 .429.243H19l.353-.854L16.853 18.146 16.499 18H9.274L4.841 11l4.433-7H16.499l.354-.146 2.5-2.5L19 .5H6.999Z"/><path fill-rule="evenodd" clip-rule="evenodd" d="M20.793 4.219l-2.427 2.427-.069.621 2.364 3.732-2.364 3.733.069.621 2.427 2.427.783-.096 3.856-6.427v-.514l-3.856-6.427-.783-.097Z"/></svg>',
                    ),
                  height: 16,
                  width: 20,
                  excavate: true,
                }}
              />
            </Suspense>
          </div>

          {/* Hint */}
          <p className="text-[11px] font-aloevera" style={hintStyle}>
            Presenta este codigo QR en la entrada
          </p>
        </div>

        {/* Bottom gold accent */}
        <div className="w-full h-1" style={accentBarStyle} />
      </div>

      {/* Action buttons */}
      <div className="flex w-full max-w-80 flex-col gap-3 sm:flex-row">
        <a
          href={`https://wa.me/?text=${whatsappText}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-2xl font-aloevera text-sm shadow-sm"
          style={{ backgroundColor: "#25D366", color: "#fff" }}
        >
          <svg
            viewBox="0 0 24 24"
            className="w-4 h-4 fill-current"
            aria-hidden="true"
          >
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.528 5.852L0 24l6.335-1.652A11.96 11.96 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.797 9.797 0 01-5.002-1.366l-.359-.213-3.722.976.994-3.624-.234-.372A9.79 9.79 0 012.182 12C2.182 6.57 6.57 2.182 12 2.182c5.43 0 9.818 4.388 9.818 9.818 0 5.43-4.388 9.818-9.818 9.818z" />
          </svg>
          Compartir
        </a>

        <button
          type="button"
          onClick={handleSaveImage}
          disabled={saveState === "saving"}
          aria-busy={saveState === "saving"}
          className="flex min-h-12 flex-1 items-center justify-center gap-2 rounded-2xl py-3.5 font-aloevera text-sm shadow-sm transition-colors disabled:cursor-wait disabled:opacity-75"
          style={saveButtonStyle}
        >
          {saveState === "saving" ? (
            <LoaderCircle
              aria-hidden="true"
              className="size-4 animate-spin motion-reduce:animate-none"
            />
          ) : saveState === "saved" ? (
            <Check aria-hidden="true" className="size-4" />
          ) : (
            <Download aria-hidden="true" className="size-4" />
          )}
          {saveState === "saving"
            ? "Preparando imagen…"
            : saveState === "saved"
              ? "Imagen guardada"
              : "Guardar imagen"}
        </button>
      </div>

      <div className="min-h-5 w-full max-w-80 text-center text-xs">
        {saveState === "saved" && (
          <p role="status" className="text-emerald-700">
            La confirmación quedó guardada en tu dispositivo.
          </p>
        )}
        {saveState === "error" && (
          <p role="alert" className="text-red-600">
            No pudimos guardar la imagen. Intenta de nuevo.
          </p>
        )}
      </div>
    </motion.div>
  );
}
